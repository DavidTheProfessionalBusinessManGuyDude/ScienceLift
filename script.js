...
const OPINION_CACHE_KEY = "sciencelift_opinion_cache";
...
function showPage(pageId) {
    ...
    if (pageId === "mainPage") activateNav("navMain");
    if (pageId === "plannerPage") activateNav("navPlanner");
    if (pageId === "settingsPage") activateNav("navSettings");
}
...
function getSelectedDays() {
    ...
    if (splitEl) {
        const days = splitEl.value === "5" ? 5 : 4;
        if (daysEl) daysEl.value = String(days);
        return days;
    }
    return daysEl ? Number(daysEl.value || 4) : 4;
}

function getFormState() {
    return {
        days: String(getSelectedDays()),
...
function applyFormState(state) {
    ...
    const splitEl = document.getElementById("split");
    if (splitEl) {
        splitEl.value = (state.days === "5") ? "5" : "4";
    }
...
function savePlanForUser(showAlert = false) {
    ...
    const openRoutineEl = document.getElementById("open-routine-display");
    const openDietEl = document.getElementById("open-diet-display");
    const openRoutineHtml = openRoutineEl ? openRoutineEl.innerHTML.trim() : "";
    const openDietHtml = openDietEl ? openDietEl.innerHTML.trim() : "";
...
function loadSavedPlanForCurrentUser(showAlert = false) {
    ...
    try {
        ...
        const openRoutineEl = document.getElementById("open-routine-display");
        const openDietEl = document.getElementById("open-diet-display");
        if (openRoutineEl) openRoutineEl.innerHTML = saved.openRoutineHtml || "";
        if (openDietEl) openDietEl.innerHTML = saved.openDietHtml || "";
...
function getSettings() {
    try {
        ...
        return parsed || { defaultWeightUnit: "kg", defaultHeightUnit: "cm", autoSave: "on", autoResearch: "on", theme: "light" };
    } catch (err) {
        return { defaultWeightUnit: "kg", defaultHeightUnit: "cm", autoSave: "on", autoResearch: "on", theme: "light" };
    }
}
...
function getOpinionCache() {
    try {
        return JSON.parse(localStorage.getItem(OPINION_CACHE_KEY));
    } catch (err) {
        return null;
    }
}

function setOpinionCache(data) {
    localStorage.setItem(OPINION_CACHE_KEY, JSON.stringify(data));
}
...
function renderResearch() {
    ...
    const items = cache.papers.slice(0, 20).map((paper) => {
...
function renderLifterOpinions() {
    const target = document.getElementById("opinions-display");
    if (!target) return;
    const cache = getOpinionCache();
    if (!cache || !cache.items || cache.items.length === 0) {
        target.innerHTML = `
            <h3>Lifter Opinion Pulse</h3>
            <p>No lifter/community opinions synced yet.</p>
            <small>Use "Sync Lifter Opinions" to fetch current high-engagement discussions.</small>
        `;
        return;
    }

    const items = cache.items.slice(0, 15).map((item) => {
        const source = item.source || "Community";
        const score = item.score ? ` | score ${item.score}` : "";
        return `<li><a href="${item.url}" target="_blank" rel="noopener noreferrer">${item.title}</a> (${source}${score})</li>`;
    }).join("");
    ...
    target.innerHTML = `
        ...
        <p>Last updated: ${new Date(cache.updatedAt).toLocaleString()}</p>
        <ul>${items}</ul>
        <small>Source mix: Reddit fitness/bodybuilding discussions + Jeff Lippard mention threads.</small>
    `;
}
...
async function syncResearch(showAlert = false, force = false) {
    ...
    try {
        const queries = [
            ...
            "training volume hypertrophy systematic review",
            "training frequency muscle growth",
            "dietary protein resistance training meta-analysis",
            "cardio and hypertrophy interference effect",
            "mechanical tension hypertrophy review",
            "failure training hypertrophy meta-analysis",
            "set volume dose response hypertrophy",
            "rest interval hypertrophy resistance training"
        ];

        const results = await Promise.all(
            queries.map(async (q) => {
                const url = `https://api.openalex.org/works?search=${encodeURIComponent(q)}&filter=from_publication_date:2017-01-01,language:en,type:article&sort=publication_date:desc&per-page=15`;
        ...
        flattened.forEach((item) => {
            const key = item.doi || item.id || item.display_name || Math.random().toString();
        ...
        const papers = Array.from(dedupedMap.values())
...
async function syncLifterOpinions(showAlert = false, force = false) {
    const cache = getOpinionCache();
    const now = Date.now();
    const dayMs = 24 * 60 * 60 * 1000;
    if (!force && cache && now - new Date(cache.updatedAt).getTime() < dayMs) {
        renderLifterOpinions();
        return;
    }

    try {
        const urls = [
            "https://www.reddit.com/search.json?q=jeff%20lippard%20hypertrophy&sort=top&t=year&limit=20",
            "https://www.reddit.com/search.json?q=hypertrophy%20volume%20frequency&sort=top&t=year&limit=20",
            "https://www.reddit.com/r/Fitness/top.json?t=year&limit=20",
            "https://www.reddit.com/r/bodybuilding/top.json?t=year&limit=20"
        ];
        const responses = await Promise.all(urls.map((u) => fetch(u)));
        const jsons = await Promise.all(responses.map((r) => r.json()));

        const raw = jsons.flatMap((j) => (j.data?.children || []).map((c) => c.data));
        const mapped = raw
            .filter((x) => x && x.title)
            .map((x) => ({
                title: x.title,
                source: x.subreddit_name_prefixed || "Reddit",
                score: x.score || 0,
                url: x.permalink ? `https://www.reddit.com${x.permalink}` : "https://www.reddit.com"
            }));
        ...
        const items = Array.from(deduped.values()).sort((a, b) => b.score - a.score).slice(0, 50);
        ...
        renderLifterOpinions();
        if (showAlert) alert("Lifter opinions synced.");
    } catch (err) {
        renderLifterOpinions();
        if (showAlert) alert("Could not sync lifter opinions right now.");
    }
}
...
function architectPlan(showValidationAlert = false) {
    ...
    if (!w || !h) {
        ...
        const openRoutineEl = document.getElementById("open-routine-display");
        const openDietEl = document.getElementById("open-diet-display");
        if (openRoutineEl) openRoutineEl.innerHTML = "";
        if (openDietEl) openDietEl.innerHTML = "";
        return;
    }
...
function setupInstallFlow() {
    ...
    const installBtnMain = document.getElementById("installBtnMain");
    const installHelp = document.getElementById("install-help");
    const installHelpMain = document.getElementById("install-help-main");
    const installButtons = [installBtn, installBtnMain].filter(Boolean);
    const installHelps = [installHelp, installHelpMain].filter(Boolean);
    const isIOS = /iphone|ipad|ipod/i.test(navigator.userAgent);
    const isStandalone = window.matchMedia("(display-mode: standalone)").matches || window.navigator.standalone;

    const setHelp = (text) => {
        installHelps.forEach((el) => {
            el.textContent = text;
        });
    };

    if (isStandalone) {
        setHelp("App installed. Open from your home screen.");
    ...
    if (isIOS) {
        setHelp("On iPhone/iPad: Share -> Add to Home Screen.");
        installButtons.forEach((btn) => {
            btn.hidden = false;
        ...
        setHelp("Use Download App for quick home-screen access.");
        installButtons.forEach((btn) => {
            btn.hidden = false;
    ...
    installButtons.forEach((btn) => {
        btn.addEventListener("click", async () => {
            ...
            if (!deferredInstallPrompt) {
                alert("Install prompt is not ready yet. In Chrome/Edge, you can also use the browser menu and choose Install app.");
                return;
            }
...
syncLifterOpinions(false, false);
